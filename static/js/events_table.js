...
var tableData = "/static/data/complete_test_data.json";
...
// Load statelist into the date dropdown.
    statelist.sort();
    let state=document.getElementById("selectState");
    loadSelect(state,statelist);

// Load citylist into the date dropdown.
    citylist.sort();
...
function readList(criteria) {
    ...
    let newData=tableData.filter(event=>
        ...
        (siting.activitytype===criteria.activitytype || criteria.activitytype==="All" || criteria.activitytype==="Select Activity Type")
    );
